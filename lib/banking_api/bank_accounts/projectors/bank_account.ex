defmodule BankingApi.BankAccounts.Projectors.BankAccount do
  ...
  def project_money_deposited(multi, %MoneyDeposited{account_number: account_number, amount: amount, date: date}) do
    multi
    |> update_bank_account(bank_account_query(account_number: account_number),
      inc: [balance: amount]
    )
    |> Ecto.Multi.run(:transaction, fn repo, _changes ->
      bank_account = repo.one!(bank_account_query(account_number: account_number))
  ...
  def project_money_withdrawn(multi, %MoneyWithdrawn{account_number: account_number, amount: amount, date: date}) do
    multi
    |> update_bank_account(bank_account_query(account_number: account_number),
      inc: [balance: -amount]
    )
